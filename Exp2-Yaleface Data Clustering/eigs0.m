function [V, bre,D, flag] = eigs0(varargin)
...
[applyOP, applyM,bre] = getOps(A, B, n, spdB, shiftAndInvert, R, cholB, permB,...
    Amatrix, innerOpts);
if bre == 1
    V = zeros(3,3);
    D = zeros(3,3);
...
function [applyOP, applyM,bre] = getOps(A, B, n, spdB, shiftAndInvert, R, cholB, ...
...
if isempty(B) % Standard problem
    if ~shiftAndInvert% OP = A
        ...
        [applyOP,bre] = AminusSigmaISolve(A, innerOpts.sigma, Amatrix, n, applyA);
    end
else % Generalized Problem
    if ~shiftAndInvert
        if spdB % OP = R^{-T} A R^{-1} (B = R^T R)
            [applyOP,bre] = applyAwithCholB(R, permB, applyA);
        else % B not HPD, OP = B^{-1} A
            [applyOP,bre] = applyAwithInvB(B, applyA);
        end
    else % OP = (A-sigma B)^{-1}
        [applyOP,bre] = AminusSigmaBSolve(A, B, innerOpts.sigma, Amatrix, n, ...
            applyA, applyB, cholB);
...
function [applyOP,bre] = applyAwithCholB(R, permB, applyA)
if issparse(R)
...
bre = WarnIfIllConditioned(rcond(dR), 'R', []);

end

function [applyOP,bre] = applyAwithInvB(B, applyA)
if issparse(B)
    umf = matlab.internal.decomposition.builtin.UMFPACKWrapper(B, 0.1, 0.001, true, 0);
    applyOP = @(v) solve(umf, applyA(v), false);
    bre = WarnIfIllConditioned(rcond(umf), 'B', []);
else
    dB = decomposition(B, 'lu', 'CheckCondition', false);
    applyOP = @(v) dB\applyA(v);
    bre = WarnIfIllConditioned(rcond(dB), 'B', []);
end
end

function [applyOP,bre] = AminusSigmaISolve(A, sigma, Amatrix, n, applyA)
if Amatrix
    % Build A - sigma I
    if sigma == 0
        AminusSigmaI = A;
    ...
    if issparse(AminusSigmaI)
        ...
        bre = WarnIfIllConditioned(rcond(umf), 'A', sigma);
    else
        dAminusSigmaI = decomposition(AminusSigmaI, 'lu', 'CheckCondition', false);
        applyOP = @(v) dAminusSigmaI\v;
        bre = WarnIfIllConditioned(rcond(dAminusSigmaI), 'A', sigma);
    end
else
    applyOP = applyA;
end
...
function [applyOP,bre] = AminusSigmaBSolve(A, B, sigma, Amatrix, n, applyA, ...
    applyB, cholB)
if Amatrix
    if sigma == 0
        AminusSigmaB = A;
    ...
    if issparse(AminusSigmaB)
        ...
        bre = WarnIfIllConditioned(rcond(dAminusSigmaB), 'A', sigma);
    end
else
    applyOP = applyA;
end
...
function [bre] = WarnIfIllConditioned(rcondest, type, sigma)
bre = 0;
if type == 'A'
